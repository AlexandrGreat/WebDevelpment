...
import MySelect from "../../formikElements/MySelect";
...
class Body extends React.Component{
    ...
    return (
        ...
        <h4 align="center">{topicText}</h4>
            ...
            <Formik 
            initialValues={{name:'',comment:'',topic:''}} 
            validationSchema={Yup.object({
                ...
                comment:Yup.string()
                .min(3,'Comment must be 3 characters at least')
                .required('Please input your comment'),
                topic:Yup.string()
                .oneOf(['Glock 19','Desert Eagle','Thompson 1928','M 16','Negev','Spas 12','AWP'],'')
                .required('Please select your topic'),
            })}
            onSubmit={(values)=>{this.setState(prevState=>({topics:[...prevState.topics,values.topic]}));
            this.setState(prevState=>({comments:[...prevState.comments,values.comment]}));
            this.setState(prevState=>({users:[...prevState.users,values.name]}));}}
            >
                <Form>
                <MyTextInput label='Name' name='name' type='text' placeholder='sample name'/><br/>
                <MyTextInput label='Comment' name='comment' type='text' placeholder='comment area'/><br/>
